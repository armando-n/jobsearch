...
namespace JobSearch.Services.DatabaseService
{
    public class DatabaseService : Template10.Mvvm.BindableBase
    {
        ...
        public void DeleteJobRequirement(Job_Requirement jobRequirement, int jobId)
        {
            Job job;

            getConnection().Delete(jobRequirement);
            Requirements.Remove(jobRequirement);

            job = Jobs.Where(aJob => aJob.JobId == jobId).Single();
            job.Requirements.Remove(jobRequirement);
            getConnection().UpdateWithChildren(job);
        }
        ...
        public void DeleteJobInterview(Job_Interview jobInterview, int jobId)
        {
            Job job;

            getConnection().Delete(jobInterview);
            Interviews.Remove(jobInterview);

            job = Jobs.Where(aJob => aJob.JobId == jobId).Single();
            job.Interviews.Remove(jobInterview);
            getConnection().UpdateWithChildren(job);
        }
        ...
        public void DeleteJobCommunication(Job_Communication jobCommunication, int jobId)
        {
            Job job;

            getConnection().Delete(jobCommunication);
            Communications.Remove(jobCommunication);

            job = Jobs.Where(aJob => aJob.JobId == jobId).Single();
            job.Communications.Remove(jobCommunication);
            getConnection().UpdateWithChildren(job);
        }
        ...
        public void Update(Job_Communication jobCommunication) => getConnection().Update(jobCommunication);
